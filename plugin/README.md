# PM Lens — Claude Code plugin

A **hybrid** distribution layer for [PM Lens](https://github.com/flc-design/pmlens).
...
may also remove that hook (`pmlens uninstall-hooks`), but it is not required.
...
# Dev MCP config: point at LOCAL source + relocate PM Lens's ~/.pm via HOME.
cat > "$SANDBOX/dev.mcp.json" <<JSON
{ "mcpServers": { "pmlens": {
  "command": "uv",
  "args": ["run", "--project", "/ABS/PATH/TO/pm-server", "pmlens", "serve"],
...
- `env.HOME` on the bundled MCP → PM Lens's `Path.home()/.pm` resolves under
...
- **Committed (`.mcp.json`)** — release form: `uvx pm-server@0.13.0`. Pinned for
  reproducibility; `uvx --from "pm-server>=0.13.0" pm-server` for a floor instead.
...
- **Collision guard — bundle + warn + documented migration** (ADR-027). The
  ...
  it. Rejected: *detect-and-defer* (mutates user config, against PM Lens's
