...
Claude Code の MCP Server として動作する PM Lens。
...
subprocess.run([
    ...
    "pmlens",
    "--",
    shutil.which("pmlens"), "serve"
], check=True)
```

4. 完了メッセージ：
```
✓ PM Lens installed successfully!
...
SynapticLedger ADR-034 / SYNAPT-079 への PM Lens 側の対応。
...
- summaries 剪定（PMSERV-162）: `pm_memory_cleanup(summaries_keep_latest=N)` →
  ...
  しない（ファイルサイズは縮まない）。
- ambiguity window の gate 化（PMSERV-163 / ADR-044）: 削除集合が ambiguity window
...
- auto-memory の物理 ingest（PMSERV-156 / ADR-045）: `pm_memory_ingest` が
  ...
  - 書き込みは派生インデックス `memory_index` のみ。プロジェクト台帳 `memories` には
    ...
    `pm_remember`）。
  - 安全境界は scope パラメータではなく **fact-based gate**（ADR-046 = ADR-045 の改定）:
    収集したノートが「現プロジェクト自身の auto-memory ディレクトリ集合」の外にある場合
    （`scope="all"` でも `auto_memory_path` override でも同じ事象）、`force=False` の実行は
    **取り込まず拒否**する（`blocked: True`・warnings[] `auto_memory_ingest_blocked_foreign`、
    dry_run は `would_block` で予告）。パラメータは事実の代理変数にすぎず、override が
    その代理を裏切っていた（scope="project" のまま任意ディレクトリを警告ゼロで取り込めた）。
    `force=True` 実行時は事後 warnings[] `auto_memory_ingested_foreign`（2 code は排他）。
    `purge=True` が取り消しで、purge 自体は gate しない（remediation の受け皿のため）。
    purge は削除行数と projects を報告し、scope="all" では
    `auto_memory_purged_all_projects` warning を返す。
  ...
  - 消えたノートの剪定は「実際にスキャンに**成功した**ディレクトリ」に限定する。限定しないと
    project スコープの実行が他プロジェクトの行を「ファイルが無い」と判定して消す。
    列挙は `os.listdir`（正直に OSError を送出）で行う — `pathlib.glob()` は
    PermissionError を握り潰して空を返すため、読めないディレクトリが「空の成功」に
    化けて索引全行を剪定した（敵対的レビューで実証: 2行→0行・エラーなし）。
    読めなかったディレクトリは `unreadable_dirs` + warnings[] で報告し剪定対象外。
  - ディレクトリは resolve() してからスキャンし source_path も resolved で保存する
    （resolve 済み/未 resolve の文字列比較非対称は、symlink 環境で再 ingest 毎の重複
    INSERT・剪定不能・purge 空振りを生んだ）。locator は resolved と raw の両綴りを
    符号化候補に含める（CC は自分が見ているパスで符号化するため）。
  - 剪定/冪等判定と DELETE+INSERT は同一 BEGIN IMMEDIATE 内で計画・適用する
    （autocommit で先に計画すると PMSERV-162 の TOCTOU 類型が再発）。
  - dry_run は純粋なプレビュー: DB ファイル作成・WAL 化・スキーマ移行を一切行わない
    （旧実装は dry_run でも ALTER まで実行していた）。
  - purge のターゲティングは「source ディレクトリ ∈ scanned **OR** 行の project_path が
    現プロジェクトルート」— ディレクトリが消滅（符号ドリフト・リポジトリ削除）しても
    行は project_path を保持しているため project スコープの purge が機能し続ける。
  - ドリフトで同一リポが複数ディレクトリを持つ場合は (project_path, basename) で
    重複排除し最新のみ索引する（registry は新旧両方の符号を同一プロジェクトへ解決）。
  - 索引行の created_at は mtime を UTC・スペース区切り（台帳の datetime('now') と
    同書式）で記録し未来値は clamp する。ローカル 'T' 区切りは TEXT 比較で全台帳行より
    上位に並ぶ（'T' > ' '）。type 欠落ノートは 'unknown'（source 列と衝突する
    'auto_memory' は使わない）。応答キーは source_path（絶対パス）— overlay の
  ...
  - ingest は書き込みなので PM_LENS=1 では登録しない（RO 不変条件）。Lens は取り込み済み
    行の検索だけ行える — これは実装上も真: readonly ストアは global_db_path を保持し、
    読みは `mode=ro&immutable=1`（sidecar を作らないため RO 不変条件は無傷、
    未チェックポイント WAL が見えないトレードオフは Lens のプロジェクト DB 読みと同一）、
    書きは `global_readonly` でストア自身が拒否する（二重防御）。旧配線は readonly で
    global_db_path を None にしており、Lens の横断検索自体が常に空だった。
- memories 側 floor の対称化（PMSERV-164 / ADR-044）: `cleanup(keep_latest=...)` も `>= 1` を
...
def install():
    ...
    内部で `claude mcp add --scope user pmlens -- <path> serve` を実行。"""

@cli.command()
def uninstall():
    """Claude Code からMCPサーバー登録を解除する。
    内部で `claude mcp remove pmlens --scope user` を実行。"""
...
@cli.command()
def migrate():
    """pm-agent からの移行。旧 MCP 登録を解除し、新 pmlens として再登録。
    1. claude mcp remove pm-agent --scope user
    2. claude mcp add --scope user pmlens -- <path> serve
...
def update_rules_cmd(target, dry_run, all_projects):
    """CLAUDE.md / AGENTS.md の PM Lens ルールを最新版に更新（ADR-008、multi-host 対応）。"""
...
def install_mcp():
    """PM Lens を Claude Code の MCP サーバーとして登録。
    claude mcp add --scope user コマンドを使用。"""
    pm_server_path = shutil.which("pmlens")
    if pm_server_path is None:
        raise RuntimeError("pmlens command not found in PATH")
    
    try:
        subprocess.run(
            ["claude", "mcp", "add", "--scope", "user",
             "pmlens", "--", pm_server_path, "serve"],
            check=True, capture_output=True, text=True
        )
        print("✓ PM Lens registered in Claude Code (user scope)")
...
def uninstall_mcp():
    ...
    try:
        subprocess.run(
            ["claude", "mcp", "remove", "pmlens", "--scope", "user"],
            check=True, capture_output=True, text=True
        )
        print("✓ PM Lens unregistered from Claude Code")
    except subprocess.CalledProcessError:
        print("PM Lens was not registered")
    except FileNotFoundError:
        print("'claude' command not found")

def migrate_from_pm_agent():
    """pm-agent から PM Lens への移行。
    1. 旧 pm-agent の MCP 登録を解除
    2. 新 pmlens を MCP サーバーとして登録
...
2026-04-27 の運用検証で、ユーザーが Codex CLI から PM Lens を呼び出そうとした際、
...
5. **絶対パス埋め込み**: `Path(sys.executable).parent / "pmlens"` を resolve した値を書き込む。Codex のサンドボックス (PATH を絞る) でも確実に起動できる
...
これは v0.4.x からの後方互換のための保守的選択 — 既存ユーザーが `pmlens install` を打ったとき、
...
class InstallResult:
    """Outcome of (un)registering PM Lens in a single host."""
...
CLI 側 (`pmlens install`) は `_print_install_summary(summary)` ヘルパで出力を構築する。
...
**成功扱い** (`✓`)。これは「PM Lens がやるべきことをやった or やる必要がなかった」を
肯定的に表示する設計判断。
...
```python
# src/pmlens/claudemd.py (v0.5.0 backward-compat shim)
...
CLI 側 (`pmlens update-rules`) は `detection_source == "fallback"` を検出すると
...
1. **Zero Configuration** — `pip install` + `pmlens install` で完了
